...
class LevelBot(commands.Cog):
    ...
    # ==========================================
    # FINAL !LEVEL COMMAND (Sends Rank, Level, Name, XP, Next XP, Progress, Avatar)
    # ==========================================
    ...
    async def level(self, ctx, member: discord.Member = None):
        ...
        # PASS THE LEVEL TO THE DASHBOARD
        image_url = f"{dashboard_url}/get_card/{user_id}?name={clean_name}&level={current_level}&xp={int(current_xp)}&next_xp={int(next_level_xp)}&progress={progress:.2f}&avatar={avatar_url}&rank={rank}"
    ...
    async def leaderboard(self, ctx):
        ...
        for i, (user_id, data) in enumerate(sorted_users, 1):
            ...
            if member:
                ...
                embed.add_field(name=f"{medal} {member.display_name}", value=f"Level {level} • {data['xp']:,} XP", inline=False)
        await ctx.send(embed=embed)
    ...
    async def xp_slowmode_set(self, ctx, seconds: int):
        ...
        self.save_config()
        if seconds == 0:
            await ctx.send("✅ XP Slowmode **DISABLED** (0 seconds). Users can gain XP instantly on every message!")
        else:
    ...
    async def auto_setup_level_roles(self, ctx):
        ...
        for level in self.levels:
            ...
            if not role:
                ...
                try:
                    new_role = await guild.create_role(name=role_name, color=color, permissions=perms, reason=f"Auto-setup level {level} role")
                    created_roles.append(new_role.name)
                except discord.Forbidden:
                    return await ctx.send("❌ I don't have permission to create roles!")
            else:
                try:
                    await role.edit(color=self.get_role_color(level), permissions=self.get_role_permissions(level), reason="Updating role to match level requirements")
        ...
        self.save_role_data()
        embed = discord.Embed(title="✅ Level Roles Setup Complete (Stops at 100)", color=discord.Color.green())
    ...
    async def remove_all_level_roles(self, ctx):
        ...
        if str(guild.id) in self.level_roles:
            ...
            self.save_role_data()
        embed = discord.Embed(title="🗑️ Level Roles Removed (Up to 100)", color=discord.Color.red())
    ...
    async def list_level_roles(self, ctx):
        ...
        if str(guild.id) not in self.level_roles or not self.level_roles[str(guild.id)]:
            return await ctx.send("❌ No level roles configured for this server.")
        embed = discord.Embed(title="📋 Level Roles (Up to 100)", color=discord.Color.blue())
        for level, role_id in sorted(self.level_roles[str(guild.id)].items(), key=lambda x: int(x[0])):
            ...
            if role:
                ...
                embed.add_field(name=f"Level {level}", value=f"{role.mention}\n*{', '.join(perm_list) if perm_list else 'Base Chat Perms'}*", inline=False)
        await ctx.send(embed=embed)
